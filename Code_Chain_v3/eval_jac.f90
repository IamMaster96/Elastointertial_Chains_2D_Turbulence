...
!$omp shared(istokes,flag,noisex,noisey,Xc,Yc,Xc0,Yc0,vxc0,vyc0,Rx,Ry,Rx0,Ry0,vxc,vyc,vRx,vRy) &
!$omp shared(uxb,uyb,vxb,vyb,tau,taup,Rmag,Rmax,Req,delta,length,Nparticle,Nbead,Lchain)
  do i1=1,Nparticle
 ...
 !! Separation vectors (links) evolution
  ...
  !! Center of mass evolution
     vxc(i1,istokes)=vxc0(i1,istokes)-delta*(vxc0(i1,istokes)-uxb(i1,1,istokes))/(2.0d0*taup)+ & 
                          dsqrt(delta*(Req*Req)/(4.0d0*tau(istokes)))*noisex(i1,1)/(taup)       &
                        +delta/(2.0d0*4.0d0*tau(istokes)*taup)*(fspring(1)*Rx(i1,1,istokes))

     vyc(i1,istokes) =vyc0(i1,istokes)-delta*(vyc0(i1,istokes)-uyb(i1,1,istokes))/(2.0d0*taup)+ & 
                          dsqrt(delta*(Req*Req)/(4.0d0*tau(istokes)))*noisey(i1,1)/(taup)     &
                        +delta/(2.0d0*4.0d0*tau(istokes)*taup)*(fspring(1)*Ry(i1,1,istokes))
    ...
    enddo
    ilink = 1
     vRx(i1,ilink,istokes) =(uxb(i1,ilink+1,istokes) -vxc0(i1,istokes)) &
		           - 1.0d0/(4.0d0*tau(istokes))*(1.0d0*fspring(ilink)*Rx(i1,ilink,istokes) &
				-fspring(ilink+1)*Rx(i1,ilink+1,istokes))

     vRy(i1,ilink,istokes) =(uyb(i1,ilink+1,istokes) -vyc0(i1,istokes)) &
		           - 1.0d0/(4.0d0*tau(istokes))*(1.0d0*fspring(ilink)*Ry(i1,ilink,istokes) &
   ...
   do ilink = 1,Nbead-1 	
        ...
        Rx(i1,ilink,istokes) = Rx0(i1,ilink,istokes) + (delta/2.0d0)*vRx(i1,ilink,istokes) + &
                dsqrt((delta/2.0d0)*(Req*Req)/(2.0d0*tau(istokes)))*(noisex(i1,ilink+1))
        Ry(i1,ilink,istokes) = Ry0(i1,ilink,istokes) + (delta/2.0d0)*vRy(i1,ilink,istokes) + &
                dsqrt((delta/2.0d0)*(Req*Req)/(2.0d0*tau(istokes)))*(noisey(i1,ilink+1))
 else
        Rx(i1,ilink,istokes) = Rx0(i1,ilink,istokes) + (delta/2.0d0)*vRx(i1,ilink,istokes) + &
  ...
  !! Center of mass evolution
     vxc(i1,istokes)=vxc0(i1,istokes)-delta*(vxc0(i1,istokes)-uxb(i1,1,istokes))/(2.0d0*taup)+ & 
                          dsqrt(delta*(Req*Req)/(2.0d0*tau(istokes)))*noisex(i1,1)/(taup)       &
                        + delta/(1.0d0*4.0d0*tau(istokes)*taup)*(fspring(1)*Rx(i1,1,istokes))

     vyc(i1,istokes) =vyc0(i1,istokes)-delta*(vyc0(i1,istokes)-uyb(i1,1,istokes))/(2.0d0*taup)+ & 
                          dsqrt(delta*(Req*Req)/(2.0d0*tau(istokes)))*noisey(i1,1)/(taup)     &
                        + delta/(1.0d0*4.0d0*tau(istokes)*taup)*(fspring(1)*Ry(i1,1,istokes))
   ...
   do ilink = 1,Nbead-1 	
        ...
        Rx(i1,ilink,istokes) = Rx0(i1,ilink,istokes) + (delta)*vRx(i1,ilink,istokes) + &
                dsqrt((delta)*(Req*Req)/(2.0d0*tau(istokes)))*(noisex(i1,ilink+1))
        Ry(i1,ilink,istokes) = Ry0(i1,ilink,istokes) + (delta)*vRy(i1,ilink,istokes) + &
                dsqrt((delta)*(Req*Req)/(2.0d0*tau(istokes)))*(noisey(i1,ilink+1))
 else
        Rx(i1,ilink,istokes) = Rx0(i1,ilink,istokes) + (delta)*vRx(i1,ilink,istokes) + &
