  ...
  if(nrunpart.eq.1) then
    ...
    do istokes = 1,nstokes
	    ...
	    do ilink = 1 ,Nbead-1
    		...
    		Rx(i1,ilink,istokes) = Rini*(ran(ilink))/dsqrt((ran(ilink))**2.0d0+(1.0d0-ran(ilink))**2.0d0)
    		Ry(i1,ilink,istokes) = Rini*(1.0d0-ran(ilink))/dsqrt((ran(ilink))**2.0d0+(1.0d0-ran(ilink))**2.0d0)
	...
	open(unit=fno,file='tau'//trim(adjustl(fname1))//'/partstate.in',status='unknown')
    	read(fno,formp) (Xp(i1,istokes),i1=1,Nparticle)
	read(fno,formp) (Yp(i1,istokes),i1=1,Nparticle)
	do ilink = 1 ,Nbead-1
...
!    Xp = pi+0.10456432211924;Yp = pi+0.861819520612071;
 ...
 subroutine beadpositions
    ...
    integer::ip,ilink,ibead, k,j
    double precision :: sumRx,sumRx

do istokes = 1,nstokes
!$omp parallel do default (none) &
!$omp private(ip,ilink,j,k,sumRx,sumRy) &
!$omp shared(istokes,Nbead,Nparticle,Xb,Yb,Xc,Yc,Rx,Ry)
    do ip = 1,Nparticle

	sumRx=0.0d0; sumRy = 0.0d0;
	do j = 2, Nbead
	  do k = 1,j-1
	    sumRx=sumRx+Rx(ip,k,istokes)
	    sumRy=sumRy+Ry(ip,k,istokes)	
	  enddo
	enddo

      	Xb(ip,1,istokes)=modulo(Xc(ip,istokes)-1.0d0/Nbead*(sumRx),length)
      	Yb(ip,1,istokes)=modulo(Yc(ip,istokes)-1.0d0/Nbead*(sumRy),length)

	do ilink = 1, Nbead-1
	   Xb(ip,ilink+1,istokes)=modulo(Xb(ip,ilink,istokes)+Rx(ip,ilink,istokes),length)
	   Yb(ip,ilink+1,istokes)=modulo(Yb(ip,ilink,istokes)+Ry(ip,ilink,istokes),length)	   
	enddo
...
!$omp shared(uxint1,uxint2,uyint1,uyint2,length,Nparticle)
    do i1 = 1,Nparticle

      do ibead = 1, Nbead

        x = Xb(i1,ibead,istokes) 
        y = Yb(i1,ibead,istokes)
	intx=floor(x/dx)+1
...
!$omp end parallel do
 enddo
  ...
  subroutine eval_lam_interp
    use mod_2dflu
    implicit none
    integer::i2,i,j,ip1,jp1,ip2,jp2,im1,jm1,i1,intx,inty
    real*8::x,y,xi,yj,xip1,yjp1,xip2,yjp2,xim1,yjm1
    real*8::df00,df01,df10,df11
...
!$omp private(df00,df01,df10,df11) &
!$omp shared(istokes,n1,Xp,Yp,Rx,Ry,dx,dy,ukx,uky) &
!$omp shared(dxux,dxuy,dyux,lamint,lam1int,lam2int,length,Nparticle) &
!$omp shared(dxux1,dxuy1,dyux1,dxux2,dxuy2,dyux2)
    do i1 = 1,Nparticle
...
!! Center of mass of Dumbbell
       ...
       dxux(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11 
       ...
       dxuy(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11
       ...
       dyux(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11

    ! Determinant of strain tensor
    dxux(i1,istokes) = dxux(i1,istokes)/(dx*dy); 
    dyux(i1,istokes) = dyux(i1,istokes)/(dx*dy);
    dxuy(i1,istokes) = dxuy(i1,istokes)/(dx*dy);
    lamint(i1,istokes) = -(dxux(i1,istokes)**2.0d0) - dxuy(i1,istokes)*dyux(i1,istokes); 
    !! Use incompressibility, dyuy = -dxux

!! Bead 1
       x = modulo(Xp(i1,istokes)+Rx(i1,istokes)/2.0d0,length); 
       y = modulo(Yp(i1,istokes)+Ry(i1,istokes)/2.0d0,length); 
       i=floor(x/dx)+1; j=floor(y/dy)+1;
       xi = dble(i-1)*dx; yj = dble(j-1)*dy;
       xip1 = xi + dx; yjp1 = yj + dy; 
       ip1 = floor(modulo(xip1,length)/dx)+1; jp1 = floor(modulo(yjp1,length)/dy)+1;
...
!       Gx = GXp(i1,istokes); Gy = GYp(i1,istokes); 
!       Gxi = Gintx*dx; Gyj = Ginty*dy;
!       Gxip1 = Gxi + dx; Gyjp1 = Gyj + dy; 
!       Gi = Gintx; Gj = Ginty; 
!       Gip1 = Gintx+1; Gjp1 = Ginty+1;

       df00 = (ukx(ip1,j) - ukx(im1,j))/(2.0d0*dx)
       df01 = (ukx(ip1,jp1) - ukx(im1,jp1))/(2.0d0*dx)
       df10 = (ukx(ip2,j) - ukx(i,j))/(2.0d0*dx)
       df11 = (ukx(ip2, jp1) - ukx(i, jp1))/(2.0d0*dx)
       dxux1(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11 
       ...
       df10 = (uky(ip2,j) - uky(i,j))/(2.0d0*dx)
       df11 = (uky(ip2, jp1) - uky(i, jp1))/(2.0d0*dx)
       dxuy1(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11
       ...
       df10 = (ukx(ip1,jp1) - ukx(ip1,jm1))/(2.0d0*dx)
       df11 = (ukx(ip1, jp2) - ukx(ip1, j))/(2.0d0*dx)
       dyux1(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11

    ! Determinant of strain tensor
    dxux1(i1,istokes) = dxux1(i1,istokes)/(dx*dy); 
    dyux1(i1,istokes) = dyux1(i1,istokes)/(dx*dy);
    dxuy1(i1,istokes) = dxuy1(i1,istokes)/(dx*dy);
    lam1int(i1,istokes) = -(dxux1(i1,istokes)**2.0d0) - dxuy1(i1,istokes)*dyux1(i1,istokes); 
    !! Use incompressibility, dyuy = -dxux
...
!       Gip1 = Gintx+1; Gjp1 = Ginty+1;
       ...
       df10 = (ukx(ip2,j) - ukx(i,j))/(2.0d0*dx)
       df11 = (ukx(ip2, jp1) - ukx(i, jp1))/(2.0d0*dx)
       dxux2(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11 
       ...
       df10 = (uky(ip2,j) - uky(i,j))/(2.0d0*dx)
       df11 = (uky(ip2, jp1) - uky(i, jp1))/(2.0d0*dx)
       dxuy2(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11
       ...
       df10 = (ukx(ip1,jp1) - ukx(ip1,jm1))/(2.0d0*dx)
       df11 = (ukx(ip1, jp2) - ukx(ip1, j))/(2.0d0*dx)
       dyux2(i1,istokes) = (xip1 - x)*(yjp1 - y)*df00 + &
            (xip1 - x)*(y - yj)*df01 + &
            (x - xi)*(yjp1 - y)*df10 + &
            (x - xi)*(y - yj)*df11

    ! Determinant of strain tensor
    dxux2(i1,istokes) = dxux2(i1,istokes)/(dx*dy); 
    dyux2(i1,istokes) = dyux2(i1,istokes)/(dx*dy);
    dxuy2(i1,istokes) = dxuy2(i1,istokes)/(dx*dy);
    lam2int(i1,istokes) = -(dxux2(i1,istokes)**2.0d0) - dxuy2(i1,istokes)*dyux2(i1,istokes); 
    !! Use incompressibility, dyuy = -dxux
    enddo
!$omp end parallel do
 enddo
