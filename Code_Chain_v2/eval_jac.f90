...
!$omp shared(istokes,flag,noisex,noisey,Xc,Yc,Xc0,Yc0,vxc0,vyc0,Rx,Ry,Rx0,Ry0,vRx0,vRy0,vxc,vyc,vRx,vRy) &
!$omp shared(uxb,uyb,vxb,vyb,tau_inertia,tau,Rmag,Rmax,Req,delta,length,Nparticle,Nbead,Lchain)
  do i1=1,Nparticle
  ...
  !! case of flag = 2: initial step with delta/2 advance
   ...
   !! Storing previous CM locations and separation vectors
     ...
     vxc0(i1,istokes)=vxc(i1,istokes)
     vyc0(i1,istokes)=vyc(i1,istokes)
   do ilink = 1, Nbead-1
     vRx0(i1,ilink,istokes)=vRx(i1,ilink,istokes)
     vRy0(i1,ilink,istokes)=vRy(i1,ilink,istokes)
     Rx0(i1,ilink,istokes) = Rx(i1,ilink,istokes)
     Ry0(i1,ilink,istokes) = Ry(i1,ilink,istokes)
   enddo

  !! Center of mass evolution
     vxc(i1,istokes)=vxc0(i1,istokes)-delta*(sum(vxb(i1,:,istokes))-sum(uxb(i1,:,istokes)))/(2.0d0*dfloat(Nbead)*tau_inertia(istokes))+ & 
                          dsqrt(delta*(Req*Req)/(4.0d0*tau(istokes)))*sum(noisex(i1,:))/(dfloat(Nbead)*tau_inertia(istokes))

     vyc(i1,istokes) =vyc0(i1,istokes)-delta*(sum(vyb(i1,:,istokes))-sum(uyb(i1,:,istokes)))/(2.0d0*dfloat(Nbead)*tau_inertia(istokes))+ & 
                          dsqrt(delta*(Req*Req)/(4.0d0*tau(istokes)))*sum(noisey(i1,:))/(dfloat(Nbead)*tau_inertia(istokes))     
 ...
 !! Separation vectors (links) evolution
    ...
    do ilink = 2, Nbead-2
     vRx(i1,ilink,istokes) =vRx0(i1,ilink,istokes)-(delta/2.0d0*tau_inertia(istokes))*(vxb(i1,ilink+1,istokes)-uxb(i1,ilink+1,istokes) - vxb(i1,ilink,istokes)+uxb(i1,ilink,istokes)) &
		           - delta/(8.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Rx(i1,ilink,istokes) &
				-fspring(ilink+1)*Rx(i1,ilink+1,istokes)-fspring(ilink-1)*Rx(i1,ilink-1,istokes))       &
                                +dsqrt((delta/2.0d0)*(Req*Req)/(2.0d0*tau(istokes)))*(noisex(i1,ilink+1) - noisex(i1,ilink))/tau_inertia(istokes)

     vRy(i1,ilink,istokes) =vRy0(i1,ilink,istokes)-(delta/2.0d0*tau_inertia(istokes))*(vyb(i1,ilink+1,istokes)-uyb(i1,ilink+1,istokes) - vyb(i1,ilink,istokes)+uyb(i1,ilink,istokes)) &
		           - delta/(8.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Ry(i1,ilink,istokes) &
				-fspring(ilink+1)*Ry(i1,ilink+1,istokes)-fspring(ilink-1)*Ry(i1,ilink-1,istokes))       &
                                +dsqrt((delta/2.0d0)*(Req*Req)/(2.0d0*tau(istokes)))*(noisey(i1,ilink+1) - noisey(i1,ilink))/tau_inertia(istokes)
    enddo
    ilink = 1
     vRx(i1,ilink,istokes) = vRx0(i1,ilink,istokes)-(delta/2.0d0*tau_inertia(istokes))*(vxb(i1,ilink+1,istokes)-uxb(i1,ilink+1,istokes) - vxb(i1,ilink,istokes)+uxb(i1,ilink,istokes)) &
		           - delta/(8.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Rx(i1,ilink,istokes) &
  ...
  !! Center of mass evolution
    ...
    vxc(i1,istokes)=vxc0(i1,istokes)-delta*(sum(vxb(i1,:,istokes))-sum(uxb(i1,:,istokes)))/(dfloat(Nbead)*tau_inertia(istokes))+ & 
                          dsqrt(delta*(Req*Req)/(2.0d0*tau(istokes)))*sum(noisex(i1,:))/(dfloat(Nbead)*tau_inertia(istokes))

    vyc(i1,istokes)=vyc0(i1,istokes)-delta*(sum(vyb(i1,:,istokes))-sum(uyb(i1,:,istokes)))/(dfloat(Nbead)*tau_inertia(istokes))+ & 
                          dsqrt(delta*(Req*Req)/(2.0d0*tau(istokes)))*sum(noisey(i1,:))/(dfloat(Nbead)*tau_inertia(istokes))
 ...
 !! Separation vectors (links) evolution
    ...
    !! Cal the total deterministic force
    do ilink = 2, Nbead-2
     vRx(i1,ilink,istokes) =vRx0(i1,ilink,istokes)-(delta/tau_inertia(istokes))*(vxb(i1,ilink+1,istokes)-uxb(i1,ilink+1,istokes) - vxb(i1,ilink,istokes)+uxb(i1,ilink,istokes)) &
		           - delta/(4.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Rx(i1,ilink,istokes) &
				-fspring(ilink+1)*Rx(i1,ilink+1,istokes)-fspring(ilink-1)*Rx(i1,ilink-1,istokes))       &
                                +dsqrt((delta)*(Req*Req)/(2.0d0*tau(istokes)))*(noisex(i1,ilink+1) - noisex(i1,ilink))/tau_inertia(istokes)

     vRy(i1,ilink,istokes) =vRy0(i1,ilink,istokes)-(delta/tau_inertia(istokes))*(vyb(i1,ilink+1,istokes)-uyb(i1,ilink+1,istokes) - vyb(i1,ilink,istokes)+uyb(i1,ilink,istokes)) &
		           - delta/(4.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Ry(i1,ilink,istokes) &
				-fspring(ilink+1)*Ry(i1,ilink+1,istokes)-fspring(ilink-1)*Ry(i1,ilink-1,istokes))       &
                                +dsqrt((delta)*(Req*Req)/(2.0d0*tau(istokes)))*(noisey(i1,ilink+1) - noisey(i1,ilink))/tau_inertia(istokes)
    enddo
    ilink = 1
     vRx(i1,ilink,istokes) = vRx0(i1,ilink,istokes)-(delta/tau_inertia(istokes))*(vxb(i1,ilink+1,istokes)-uxb(i1,ilink+1,istokes) - vxb(i1,ilink,istokes)+uxb(i1,ilink,istokes)) &
		           - delta/(4.0d0*tau(istokes)*tau_inertia(istokes))*(2.0d0*fspring(ilink)*Rx(i1,ilink,istokes) &
