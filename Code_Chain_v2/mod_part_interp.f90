...
!  real*8,allocatable,dimension(:,:)::Gdxux,Gdxuy,Gdyux
  real*8,allocatable,dimension(:)::tau_chain,tau,tau_inertia
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!(mohit_code_editing)
        ...
        do  istokes=1,nstokes
        do i1=1,Nparticle
                vxc(i1,istokes)=sum(vxb(i1,:,istokes))/Nbead
                vyc(i1,istokes)=sum(vyb(i1,:,istokes))/Nbead
        do ilink=1,Nbead-1
                vRx(i1,ilink,istokes)=vxb(i1,ilink+1,istokes)-vxb(i1,ilink,istokes)
                vRy(i1,ilink,istokes)=vyb(i1,ilink+1,istokes)-vxb(i1,ilink,istokes)
        end do
        end do
        end do
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!(mohit_code_editing)
	...
	do ilink = 1 ,Nbead-1
		...
		read(fno,formp) (Ry(i1,ilink,istokes),i1=1,Nparticle)
                read(fno,formp) (vRx(i1,ilink,istokes),i1=1,Nparticle)
                read(fno,formp) (vRy(i1,ilink,istokes),i1=1,Nparticle)
	enddo
	close(fno)
	 ...
	 write(fname2,'(g8.0)')  ilink
   	 ...
   	 open(unit=fno,file='tau'//trim(adjustl(fname1))//'/Rx' &
    	 ...
    	 open(unit=fno,file='tau'//trim(adjustl(fname1))//'/Ry' &
			//trim(adjustl(fname2))//'track.out',status='unknown')
        fno = fno + 1
   	 open(unit=fno,file='tau'//trim(adjustl(fname1))//'/vRx' &
			//trim(adjustl(fname2))//'track.out',status='unknown')
    	 fno = fno + 1
    	 open(unit=fno,file='tau'//trim(adjustl(fname1))//'/vRy' &
			//trim(adjustl(fname2))//'track.out',status='unknown')
	enddo
	do ibead = 1, Nbead
...
!$omp shared(istokes,Nbead,Nparticle,Xb,Yb,Xc,Yc,Rx,Ry,length)
	...
	enddo
      	...
      	Xb(i1,1,istokes)=modulo(Xc(i1,istokes)-1.0d0/Nbead*(sumRx),length)
      	Yb(i1,1,istokes)=modulo(Yc(i1,istokes)-1.0d0/Nbead*(sumRy),length)
...
!$omp shared(istokes,Nbead,Nparticle,vxb,vyb,vxc,vyc,vRx,vRy,length)
	...
	enddo
      	...
      	vxb(i1,1,istokes)=vxc(i1,istokes)-1.0d0/Nbead*(sumVx)
      	vyb(i1,1,istokes)=vyc(i1,istokes)-1.0d0/Nbead*(sumVy)
