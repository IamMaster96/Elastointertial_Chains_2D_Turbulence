...
!! % --- Set the number of threads
  ...
  allocate(vRx0(Nparticle,Nbead-1,nstokes),vRy0(Nparticle,Nbead-1,nstokes))
  allocate(uxb(Nparticle,Nbead,nstokes),uyb(Nparticle,Nbead,nstokes),vyb(Nparticle,Nbead,nstokes),vxb(Nparticle,Nbead,nstokes))
  allocate(vxc(Nparticle,nstokes),vyc(Nparticle,nstokes))
  allocate(vxc0(Nparticle,nstokes),vyc0(Nparticle,nstokes))
...
!  allocate(Gdxux(Nparticle,nstokes),Gdxuy(Nparticle,nstokes),Gdyux(Nparticle,nstokes))
  allocate(tau_chain(nstokes),tau(nstokes),tau_inertia(nstokes))
 ...
 !       write(*,*) counter*delta,0.5d0*sum(E_Omega(:,1)),-vis*sum(E_Omega(:,2))
    ...
    if (particl) then
     ...
     do ilink = 1, Nbead-1
        ...
        fno = fno + 1
   	 write(fno,formp) (vRx(i1,ilink,istokes),i1=1,Nparticle)
    	 fno = fno + 1
    	 write(fno,formp) (vRy(i1,ilink,istokes),i1=1,Nparticle)
	enddo
	do ibead = 1, Nbead
    	 fno = fno + 1
   ...
   enddo
    ...
    do ispectra= 1,nshell
      write(225,*) dfloat(ispectra),E_Omega(ispectra,1)!,E_Omega(ispectra,2),E_Omega(ispectra,3)
    enddo
    close(225)
  enddo
  ...
  if(particl) then
   ...
   do istokes = 1,nstokes
    ...
    open(unit=fno,file='tau'//trim(adjustl(fname1))//'/partstate.out',status='unknown')
    	write(fno,formp) (Xc(i1,istokes),i1=1,Nparticle)
	write(fno,formp) (Yc(i1,istokes),i1=1,Nparticle)
        write(fno,formp) (vxc(i1,istokes), i1=1,Nparticle)
        write(fno,formp) (vyc(i1,istokes), i1=1,Nparticle)
	do ilink = 1 ,Nbead-1
    		write(fno,formp) (Rx(i1,ilink,istokes),i1=1,Nparticle)
		write(fno,formp) (Ry(i1,ilink,istokes),i1=1,Nparticle)
                write(fno,formp) (vRx(i1,ilink,istokes), i1=1,Nparticle)
